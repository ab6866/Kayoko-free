...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        // Authorization has been removed: Kayoko is permanently free, so this empty state
        // is never installed. The property stays nil and every check below resolves to NO.
...
- (void)setShowIconAndTime:(BOOL)showIconAndTime {
    _showIconAndTime = showIconAndTime;
    [[self historyListViewController] setShowIconAndTime:showIconAndTime];
...
- (BOOL)isAuthorizationRequired {
    // Authorization removed: never block the panel behind a purchase check.
    return NO;
}
